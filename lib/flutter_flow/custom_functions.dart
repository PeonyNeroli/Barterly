...
bool? checkFavOrNot(
  ...
  if (favList != null && favList.isNotEmpty) {
    for (var element in favList) {
      if (element["_id"].toString() == courseId!.toString()) {
        value = true;
        break;
      }
    }
...
bool checkBuyCourseMyCourseListFunction(
  String courseId,
  List<dynamic> listCourseId,
) {
  if (listCourseId == null || listCourseId.isEmpty) {
    return false;
  }

  return listCourseId.any((element) =>
      element["course"] != null &&
      element["course"]["_id"].toString() == courseId.toString());
}

String convertUtcToLocal(String utcTimeString) {
  // DateTime utcTime = DateTime.parse(utcTimeString);
  // DateTime localTime = utcTime.toLocal();
  // String formattedLocalTime = DateFormat('kk:mm').format(localTime);
...
String? convertDate(String? date) {
  ...
  DateTime inputDate = DateFormat("yyyy-MM-dd hh:mm:ss").parse(inputDateString);
...
int stringToInt(String percentage) {
  ...
  try {
    ...
    // Handle parsing errors (e.g., if the string is not a valid integer)
    print("Error parsing percentage: $e");
    return 0; // Or handle the error in your desired way
  }
}

bool checkBuyCourseCompleteFunction(
  String courseId,
  List<dynamic> listCourseId,
) {
  if (listCourseId == null || listCourseId.isEmpty) {
  ...
  return listCourseId.any((element) =>
      ...
      element["courseId"]["_id"].toString() == courseId.toString());
}
...
String reviewCounter(
  dynamic ratingCounts,
  String number,
) {
  switch (number) {
    case 1:
      return ratingCounts['1'];
    case 2:
      return ratingCounts['2'];
    case 3:
      return ratingCounts['3'];
    case 4:
      return ratingCounts['4'];
    case 5:
      return ratingCounts['5'];
    default:
      return "";
  }
}
